...
$date = date("Y-m-d");
  ...
  $account = $_SESSION['account'];
    ...
    $acDailyFgActivitySQL = "SELECT * FROM ac_daily_fg_activities WHERE UNIT_CODE = '$account'";
    $acDailyFgActivityResult = mysqli_query($conn, $acDailyFgActivitySQL);
    $totalRecords = mysqli_num_rows($acDailyFgActivityResult);  
    $remark = "NIL";
...
<main id="main" class="main" 
  style="
    ...
    <div class="pagetitle">
      <h1  style="font-weight:normal; font-size:125%;">AC DAILY FG ACTIVITIES</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item"><a href="unitAdminHome">Home</a></li>
          <li class="breadcrumb-item active"><a href="unitAdminOpms">Dashboard</a></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card" style="padding:20px; padding-bottom:80px; border-radius:0px;">
            <div class="card-body">
              <h5 class="card-title">AC DAILY FG ACTIVITIES - <?php echo $account." "; ?></h5>
              <p>
                Click the buttons to see ac daily fg activities and to add new fg activity.
              </p>
             ...
             <!-- fullscreen Modal -->
              <button type="button"  style="border-radius:1px; margin-top:20px; margin-right:5px; padding:15px;" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#fullscreenModal">
                CLICK TO VIEW ALL AC DAILY FG ACTIVITIES
              </button>
                ...
                <div class="modal-dialog modal-fullscreen" style="border-radius:1px;">
                  <div class="modal-content" style="border-radius:1px;">
                    <div class="modal-header" style="border-radius:1px;">
                      <h5 class="modal-title">AC DAILY FG ACTIVITIES - <?php echo $account." "; ?></h5>
            ...
            <div class="table-responsive" style="boder">
              <table class="table datatable table-striped second table-hover" style="text-transform:uppercase; font-size:80%; background-color:white;">
                ...
                <tbody>
                    ...
                    while($acDailyFgActivityFetch=mysqli_fetch_assoc($acDailyFgActivityResult))
            {
            ?>
                <tr id='userRow' style='text-transform: uppercase; font-size:80%;'>
                <td><?php echo $acDailyFgActivityFetch['AC_TYPE']?></td>
                <td><?php echo $acDailyFgActivityFetch['TAIL_NO']?></td>
                <td><?php echo $acDailyFgActivityFetch['SORTIES']?></td>
                <td><?php echo $acDailyFgActivityFetch['PILOTS']?></td>
                <td><?php echo $acDailyFgActivityFetch['MSG_TYPE']?></td>
                <td><?php echo $acDailyFgActivityFetch['FUEL_CONSUMED']?></td>
                <td><?php echo $acDailyFgActivityFetch['HOURS_FLOWN']?></td>
                <td><?php echo $acDailyFgActivityFetch['ROUTE_FLOWN']?></td>
                <td><?php echo $acDailyFgActivityFetch['ACTIVITY_DATE']?></td>
                <td><?php echo $acDailyFgActivityFetch['UNIT_CODE']?></td>
                <td><?php echo $acDailyFgActivityFetch['REMARK']?></td>
                <td><a style="color:black" href='unitAdminDeleteAcDailyFgActivity?acDailyFgActivityId=<?php echo $acDailyFgActivityFetch['ID'];?>' type='submit' onClick='javascript:return confirm("\nARE YOU SURE YOU WANT TO DELETE THIS RECORD??? DELETING THIS RECORD REMOVES IT FROM THE DATABASE. CLICK OK TO DELETE AND CANCEL TO CANCEL .....\n");'><i class='bi bi-trash' id='deleteButton'></i></a></td>
              </tr>
            <?php
            }
            ?>
             ...
             <!-- large Modal -->
              <button type="button"  style="border-radius:1px; margin-top:20px; margin-right:5px; padding:15px;" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#largeModal">
                CLICK TO ADD NEW AC DAILY FG ACTIVITY...
              </button>
              <div class="modal fade" id="largeModal" tabindex="-1" style="border-radius:1px;">
                <div class="modal-dialog modal-lg" style="border-radius:1px;">
                  <div class="modal-content" style="border-radius:1px;">
                    <div class="modal-header" style="border-radius:1px;">
                      <h5 class="modal-title">NEW AC DAILY FG ACTIVITY</h5>
                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body"
                      style="
                    ...
                    <!-- Multi Columns Form -->
              <form method="post" action="unitAdminProcessNewAcDailyFgActivity" class="row g-3" style="padding-left:10px; padding-right:10px; padding-top:20px; padding-bottom:20px;">
                <div class="col-md-4">
